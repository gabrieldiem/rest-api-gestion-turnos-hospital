...
module RoutesResourceTurnosReservadosPacientes
  ...
  def self.get_turnos_historial(app)
    app.get '/pacientes/:dni/historial' do
      logger.debug("GET /pacientes/#{params[:dni]}/turnos-reservados")

      turnos = turnero.obtener_historial_turno_del_paciente_por_dni(params[:dni])
